...
- 関連Issueをマージ時に自動クローズしたい場合は，PR本文に `Closes #123` / `Fixes #123` / `Resolves #123` のいずれかを書く
- `関連Issue: #123` だけではIssueは自動クローズされない
- 自動クローズは，PRがデフォルトブランチ（このリポジトリでは `main`）にマージされたときに有効
- 迷ったらPRテンプレートの `Closes #123` をそのまま使う
