...
- 関連Issueをマージ時に自動クローズしたい場合は，PR本文に `Closes #<Issue番号>` / `Fixes #<Issue番号>` / `Resolves #<Issue番号>` のいずれかを書く（番号は対象Issueのものに置き換える。例: `Closes #45`）
- `関連Issue: #123` だけではIssueは自動クローズされない
- 自動クローズが効くのは，PRの対象（base）ブランチがデフォルトブランチ（このリポジトリでは `main`）の場合のみ。別ブランチ向けPRを後から `main` に取り込んでも自動クローズされない
- 迷ったらPRテンプレートの `Closes` 形式をそのまま使う
