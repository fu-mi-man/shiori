...
| F-6 | 失敗時（503等）は自動で1回リトライ → それでも失敗ならトースト + 再試行ボタン。手動作成へのフォールバック導線も出す。**リトライ対象は生成呼び出しのみ**（登録まで成功した処理は再実行せず，重複作成を防ぐ） |
...
| N-3 | 無料枠運用中は支出自体が発生しない（枠超過は 429 で生成停止・スペンド上限は設定不可）。課金を有効化する場合は**有効化と同時に**プロバイダ側で月額スペンド上限を設定する |
...
- **再生成の上書き**: `ai_generations` に `shiori_id` と入力条件を記録し，同条件で生成し直して中身を差し替える。上書きは INSERT だけでは済まず**既存の overviews / schedules を全削除してから再INSERT**するため，`insert.ts` は新規作成と中身差し替えの両方をトランザクションで扱う（`shioriId` の有無で分岐）。再生成と手動編集の同時実行の競合制御は Phase 1 では扱わない（認証なし・URL共有型で同時操作が実質発生しないため。認証導入の Phase 2 で再検討）
- **編集済み判定**: 時刻の大小比較は精度・更新順序で誤判定し得るため，生成完了時の `shioris.updatedAt` を `ai_generations` にスナップショットとして記録し，現在の `updatedAt` との**不一致**で編集済みと判定する（登録と同一トランザクションで記録するため生成直後は必ず一致＝未編集）
...
- **上限**: 2段構え。**登録スキーマ上限**（超過で登録失敗）= 日程 ≤10日・overviews ≤10件・title / overview title / 行程 title ≤255字・overview content ≤500字・memo ≤200字。**プロンプト目標値**（表示品質のためスキーマより狭める）= title ≤50字・行程 title ≤40字・memo ≤100字。プロンプトには目標値を書き，疎通スクリプト `ai-smoke.mts` と同じ制約に揃える
...
ipHash: varchar("ip_hash", { length: 64 }).notNull(),  // HMAC-SHA-256(サーバー秘密鍵, IP)，生IPは保存しない（salt 連結より漏えい時の総当たりに強い）
userId: integer("user_id"),                            // Phase 2 から使用，NULL可
shioriId: uuid("shiori_id"),                           // 生成しおり，再生成の条件復元・編集済み判定に使う（onDelete: set null）
shioriUpdatedAt: timestamp("shiori_updated_at"),       // 生成完了時の shioris.updatedAt スナップショット，編集済み判定（§4）に使う
request: jsonb("request").notNull(),                   // 入力条件（出発日・自由記述），再生成で再利用。自由記述に個人情報が入り得るため，しおり本体と同じ3ヶ月TTLで削除し保存目的・期間をプライバシーポリシーに明記
...
| # | リスク | 対策 |
|---|---|---|
| 1 | API乱用によるコスト暴走 | IP/ユーザー単位レート制限 + 入力500字上限。無料枠運用中は支出ゼロで被害上限は枠まで。課金有効化時はスペンド上限を必ず設定（N-3） |
...
**事前に必要な作業（コード外）**: 採用プロバイダのAPIキー発行のみ（スペンド上限は課金有効化時に設定）。
